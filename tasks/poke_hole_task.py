...
class PokeHoleTask(Task):
    def __init__(self, agent_manager: 'AgentManager', num_blocks: int, place_torches_every: int = 14, notify: bool = False):
        ...
        self.commands: Dict[str, List[Command]] = {
            ...
            "return": [MoveCommand(agent_manager, move_to=MoveTo(forward=0, side=0, up=0), face_to=FaceTo(forward=1, side=0, up=0))],
    ...
    async def run(self) -> None:
        ...
        if self.notify:
            notify("Pokehole reached end for: ", self.agent_manager.label)
        await self.commands["return"][0].run()
        if self.notify:
